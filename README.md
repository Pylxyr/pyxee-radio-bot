...
length) are live-adjustable from `/settings` without a restart, or from
chat via `!setlimit` (moderators only) — e.g. `!setlimit queue_cap 100`.
Valid keys: `max_pending_per_chatter`, `request_cooldown_seconds`,
