...
# handler below — !sr and !setlimit are the only two commands with a
# required argument today. Keyed by command name (not alias) since
...
class SongRequestComponent(commands.Component):
    ...
    async def commands_list(self, ctx: commands.Context) -> None:
        p = self.bot.prefix
        await ctx.reply(
            f"{p}sr <song/URL> — queue a song  |  {p}skip — skip (mods, or your own "
            f"playing song)  |  {p}remove — pull back your own queued request  |  "
            f"{p}queue — queue length  |  {p}nowplaying — what's playing"
        )
